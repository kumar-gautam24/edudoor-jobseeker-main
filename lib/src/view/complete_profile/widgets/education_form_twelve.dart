...
class EducationTwelveBasicState extends StatefulWidget {
  final bool onlyUpdate;
  const EducationTwelveBasicState({super.key, required this.onlyUpdate});

  @override
  State<EducationTwelveBasicState> createState() => _EducationTwelveBasicStateState();
}

class _EducationTwelveBasicStateState extends State<EducationTwelveBasicState> {
  String? _board;
  String? _medium;
  String? _passingYear;
  String? _percentage;
  ...
  void update() {
    ...
    _board = jobseekerModel?.education?.the12Th?.board;
    _medium = jobseekerModel?.education?.the12Th?.medium;
    _passingYear = jobseekerModel?.education?.the12Th?.passingYear?.toString();
    _percentage = jobseekerModel?.education?.the12Th?.percentageORmarks?.toString();
                              ...
                              children: [
                                if (_showDelete == true)
                                  TextButton(
                                    onPressed: () {
                                      showEducationDialog(
                                          context, 'Delete senior secondary dialog', 'Delete senior secondary Education Details', 'Are you sure you want to delete senior secondary Education Details?',
                                          () {
                                        context.read<AuthenticationBloc>().add(
                                              AuthenticationEducationDetailsEvent(
                                                true,
                                                EducationType.twelve,
                                                clear: true,
                                              ),
                                            );
                                      });
